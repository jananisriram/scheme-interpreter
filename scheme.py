...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        validate_procedure(curr_operator)
        if isinstance(curr_operator, MacroProcedure):
            return scheme_eval(curr_operator.apply_macro(rest, env), env)
        curr_operands = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(curr_operator, curr_operands, env)
        # END PROBLEM 4
...
class Frame:
    ...
    def __init__(self, parent):
        ...
        self.bindings = {} # maps scheme symbols (python strings) to scheme values
        self.parent = parent
    ...
    def lookup(self, symbol):
        ...
        while curr_parent:
            if symbol in curr_parent.bindings:
                return curr_parent.bindings[symbol]
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 10
        #self.define() # binds smt to smt
        """
        new_frame = Frame(self)
        curr_formal = formals
        curr_val = vals
        while curr_formal and curr_val and curr_formal.rest and curr_val.rest:
            new_frame.define(curr_formal.first, curr_val.first)
            curr_formal.first, curr_formal.rest = curr_formal.rest.first, curr_formal.rest.rest
            curr_val.first, curr_val.rest = curr_val.rest.first, curr_val.rest.rest
        if curr_formal and curr_val:
            new_frame.define(curr_formal.first, curr_val.first)
        return new_frame
        """
        new_frame = Frame(self)
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        arguments_list = []
        # BEGIN PROBLEM 3
        while args and not args.rest == nil:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        #print("DEBUG: ", lambda_instance.name)
        #target.first = do_lambda_form(expressions.rest, env)
        #formals = target.rest
        #body = expressions.rest
        #lambda_instance = LambdaProcedure(formals, body, env)
        # where do I use do_lambda_form??
        #print("DEBUG: ", name, formals, body, lambda_instance)
        ...
        body = expressions.rest
        lambda_instance = LambdaProcedure(formals, body, env)
        env.define(target.first, lambda_instance)
        #print("DEBUG: ", formals, body)
        return target.first
        # END PROBLEM 9
    else:
...
def do_lambda_form(expressions, env):
    ...
    #formals = expressions.first
    """
    expressions.first, expressions.rest = expressions.rest.first, expressions.rest.rest
    body = Pair(expressions.first, expressions.rest)
...
def do_and_form(expressions, env):
    ...
    if is_false_primitive(scheme_eval(expressions.first, env)):
        ...
        return '#t'
    elif expressions.rest.first:
        do_and_form(expressions.rest, env)
    ...
    while expressions is not nil:
        ...
        if is_false_primitive(curr_val):
            return False
...
def do_or_form(expressions, env):
    ...
    while expressions is not nil:
        if expressions.rest is not nil:
            ...
            curr_val = scheme_eval(expressions.first, env, True)
        if not is_false_primitive(curr_val):
            return curr_val
        expressions = expressions.rest
    return False
    ...
    """
    if expressions is nil:
        curr_val = scheme_eval(False, env)
    #elif len(expressions) == 0:
        #return '#f'
    else:
        if expressions.rest is not nil:
...
def make_let_frame(bindings, env):
    ...
    names = values = nil
    # BEGIN PROBLEM 14
    #print("DEBUG: ", bindings.first.first, bindings.first.rest.first)
    ...
    while bindings:
        validate_form(bindings.first, 2, 2)
        #bindings.first, bindings.rest = bindings.rest.first, bindings.rest.rest
    ...
    # issue: suite 1, case 3: it's not going into the first parameter
    # of the nested let
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        env.define(target.first, macro_instance)
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
